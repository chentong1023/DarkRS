...
from utils.build import DATASET_REGISTRY


@DATASET_REGISTRY.register()
class DarkRS(torch.utils.data.Dataset):
    def __init__(self, cfg):
        self.cfg = cfg

        seqs_path = os.path.join(self.cfg.root_gt, self.cfg.mode + '_scenes.txt')

        with open(seqs_path) as f:
            txts = f.readlines()
        self.video_list = [it[:-1] for it in txts]
        ...
        for video_name in self.video_list:
            ...
            video_path = os.path.join(self.cfg.root_gt, self.cfg.mode, video_name)
            frames_in_video = os.listdir(os.path.join(video_path, self.cfg.pos, self.cfg.img_type))
    ...
    def __getitem__(self, idx):
        ...
        gt_frames_path = os.path.join(self.cfg.root_gt, self.cfg.mode, video_name, self.cfg.pos, self.cfg.img_type, "{}")
        input_frames_path = os.path.join(self.cfg.root_gt, self.cfg.mode, video_name, "RS", self.cfg.img_type, "{}")
