...
class ResBlock(nn.Module):
    ...
    def forward(self, x):
        out = self.conv1(x)
        out1 = self.conv2(out[:, -self.side_channels:, :, :].clone())
        out[:, -self.side_channels:, :, :] = out1
        out = self.conv3(out)
        out2 = self.conv4(out[:, -self.side_channels:, :, :].clone())
        out[:, -self.side_channels:, :, :] = out2
...
class Decoder4(nn.Module):
    ...
    def forward(self, f0, f1):
        b, c, h, w = f0.shape
        embt = torch.arange(h).view(-1, 1).expand(b, 1, h, w).cuda() / h
...
class IFRNet_L(nn.Module):
    ...
    def inference(self, x, scale_factor=1.0):
        B, N, C, H, W = x.shape
        img0 = x[:,0,...]
        img1 = x[:,1,...]
    ...
    def forward(self, x, imgt):
        ...
        f0_4, f1_4 = feature_add_position(f0_4, f1_4, self.attn_num_splits, self.init_chs[-1])
        f0_4, f1_4 = self.transformer(f0_4, f1_4, attn_num_splits=self.attn_num_splits)
        ...
        # _, flow0_1 = self.teacher(imgt, img0, iter=2, test_mode=True)
        # _, flow1_1 = self.teacher(imgt, img1, iter=2, test_mode=True)
        # robust_weight0 = get_robust_weight(up_flow0_1, flow0_1, beta=0.3)
        # robust_weight1 = get_robust_weight(up_flow1_1, flow1_1, beta=0.3)
        # loss_dis = 0.01 * (self.rb_loss(2.0 * resize(up_flow0_2, 2.0) - flow0_1, weight=robust_weight0) + self.rb_loss(2.0 * resize(up_flow1_2, 2.0) - flow1_1, weight=robust_weight1))
        # loss_dis += 0.01 * (self.rb_loss(4.0 * resize(up_flow0_3, 4.0) - flow0_1, weight=robust_weight0) + self.rb_loss(4.0 * resize(up_flow1_3, 4.0) - flow1_1, weight=robust_weight1))
        # loss_dis += 0.01 * (self.rb_loss(8.0 * resize(up_flow0_4, 8.0) - flow0_1, weight=robust_weight0) + self.rb_loss(8.0 * resize(up_flow1_4, 8.0) - flow1_1, weight=robust_weight1))

        return [imgt_pred], [ft_1_, ft_2_, ft_3_], [ft_1, ft_2, ft_3],\
            [up_flow0_1, up_flow0_2, up_flow0_3, up_flow0_4], \
            [up_flow1_1, up_flow1_2, up_flow1_3, up_flow1_4]